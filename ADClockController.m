...
@interface ADClockController (PrivateAPI)
- (void)setupPlayer;
- (void)playerPlay;
- (void)playerStop;
...
@implementation ADClockController
...
- (id)init
{
	if(self = [super initWithWindowNibName:@"AlarmWindow"])
	{
		...
		lastAlarm = [[ADClockScheduler lastAlarmClone] retain];
...
- (void)dealloc
{
	...
	// Release Player and core audio device
	[player release];
...
/**
 Called when the user tries to close the window, e.g. by clicking the real red traffic-light
 close button. The window now has genuine system chrome (see RoundedWindow), so unlike before
 this can happen without going through our own Stop button.
 We must not let the window simply disappear: -stop is the only place that stops the alarm
 sound and restores system volume. So we route the close through it, and only allow the real
 close once -stop's own fade-out timer is ready to close the window itself.
**/
...
/**
 This method initializes and configures the ITunesPlayer to play the built-in default alarm
 sound. The app no longer integrates with the Music library (no track/playlist selection),
 so there's nothing to look up before the player can be created.
...
- (void)setupPlayer
{
	...
	if(!isPlayerReady)
	{
		// Initialize ITunesPlayer (it doesn't need Music library access to just play a file)
		player = [[ITunesPlayer alloc] initWithITunesData:nil];
