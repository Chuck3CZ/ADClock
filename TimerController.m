...
- (void)windowDidLoad
{
	...
	// Now that the window has a real (visible) titlebar, keep its title in sync
...
- (void)edit:(BOOL)isInitialSetup
{
	...
	[alwaysOnTopButton setState:([[self window] level] == NSStatusWindowLevel) ? NSControlStateValueOn: NSControlStateValueOff];
	
	[useAlarmVolumeButton setState:(useAlarmVolume) ? NSControlStateValueOn : NSControlStateValueOff];
...
- (IBAction)closeConfigPanel:(id)sender
{
	...
	BOOL alwaysOnTop = [alwaysOnTopButton state] == NSControlStateValueOn;
	if(alwaysOnTop)
		[[self window] setLevel:NSStatusWindowLevel];
	else
	...
	useAlarmVolume = [useAlarmVolumeButton state] == NSControlStateValueOn;
