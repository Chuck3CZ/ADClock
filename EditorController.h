...
@interface EditorController : NSWindowController
{
	...
	// Replaces the old repeatSchedule NSMatrix with 7 individually-spaced day buttons
	// (tags 0...6, same order/meaning as the old matrix cells) - see -awakeFromNib.
	NSArray *repeatDayButtons;

	// Replaces the old calPanel sheet (which forced NSDatePicker into a frame sized for
	// the ancient CalendarView bitmap) with a standard NSPopover, the native macOS pattern
	// for "click a button, a calendar appears" (e.g. Calendar.app's own date pickers).
	NSPopover *calendarPopover;
	NSDatePicker *calendarDatePicker;
